...
const WeeklyPicker = (props) => {
    ...
    for (let i = 0; i <= 6; i++) {
      ...
      tempTiles.push(
        <View  style={styles.daySliderSlot} key={dateData.format('YYYY-DD-MM')}>
          <Text style={styles.daySliderDayTitleText}>{dateData.format('dd')}</Text>
          <TouchableOpacity style={styles.daySlide} >
            <Text style={styles.dayOfMonthText}>{moment(dateData).format('DD')}</Text>
          </TouchableOpacity>
        </View>
      )
    }
  ...
  return (
      <View style={styles.daySlider}>
        <TouchableOpacity style={styles.expandCalendarButton} onPress={handleOpenExpandedPicker}>
          <FontAwesome size={28} name="calendar-o" color={THEME.COLORS.secondary} />
        </TouchableOpacity>
        {sliderTiles}
